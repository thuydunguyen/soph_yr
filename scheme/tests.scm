...
(* size (/ circumference radius) (/ 1 pi))
; expect 4
...
(- 451 (* 34 21) (- 23 1.5) (* 9.8 2.5))
; expect -309

(define (root mins maxs func tol)
  (define half (/ (+ mins maxs) 2))
  (cond 
    ((< (abs (func half)) tol) half)
    ((< (func half) 0) (root half maxs func tol))
    ((> (func half) 0) (root mins half func tol))))
(define func (lambda (x) (- x 5)))
(root 4 10 func 0.1)
; expect 4.9375

(define func (lambda (x) (* x (- x 1) (+ x 2))))
(func 2)
; expect 8
...
(define (f x)
  (* (sum-of-squares(+ x 1) (* x 2)) (func 2) (func 3)))
; expect f

(f 5)
; expect 32640

(define (up-or-down x y)
  (cond
    ((> x y) "down")
    ((< x y) "up")
    (else "match")))
...
(begin (print "Suffering") "Student")
; expect "Student"

(define y (begin (define func (lambda (x) (+ x 1))) func))
; expect y
(y 5)
; expect 6
...
(let ((a '13th) (b 'Friday) (c 'The)) (list b c a))
; expect (friday the 13th)
